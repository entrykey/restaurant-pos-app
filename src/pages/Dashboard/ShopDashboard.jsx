...
const ShopDashboard = () => {
    ...
    const stats = [
        { label: "Today's Sales", value: formatCurrency(data?.totalSales || 0), icon: ShoppingBag, color: 'text-blue-600', bg: 'bg-blue-50', link: `/dashboard/sales?startDate=${today}&endDate=${today}` },
        { label: 'Today Profit', value: formatCurrency(data?.totalProfit || 0), icon: TrendingUp, color: (data?.totalProfit || 0) >= 0 ? 'text-green-600' : 'text-red-600', bg: (data?.totalProfit || 0) >= 0 ? 'bg-green-50' : 'bg-red-50', link: `/dashboard/operating-expenses?tab=timeline&startDate=${today}&endDate=${today}` },
        { label: 'Pay In (Customers)', value: formatCurrency(data?.payIn || 0), icon: Users, color: 'text-orange-600', bg: 'bg-orange-50', link: '/dashboard/pay-in', tooltip: 'Outstanding customer balances (all time)' },
