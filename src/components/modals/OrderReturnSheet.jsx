...
import { X, RotateCcw, Plus, Trash2, Search, CheckCircle2, AlertCircle, ShoppingBag } from 'lucide-react';
...
const OrderReturnSheet = ({ isOpen, onClose, order, onSuccess }) => {
    ...
    const totalReturn = returnedItems.reduce((acc, item) => acc + (item.price * item.quantity), 0);
    ...
    const handleContinueToPOS = () => {
        ...
        // Set global exchange state
        setIsExchange(true);
        setExchangeCredit(totalReturn);
        setGlobalReturnedItems(returnedItems);
        setOriginalOrderId(order._id);

        // Navigate to the Direct Sale (Takeaway) POS route for all exchanges
        // This ensures an empty cart and a fresh direct sale context as requested
        const targetRoute = '/takeaway';
                    ...
                    <section className="space-y-4">
                        ...
                        <div className={`rounded-3xl border overflow-hidden ${theme.borderLight}`}>
                            {(order.items || []).map((item, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`p-4 flex items-center justify-between group transition-colors ${isAdded ? 'bg-orange-50/50 dark:bg-orange-900/10' : 'hover:bg-gray-50 dark:hover:bg-white/5'}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className={`text-xs font-bold ${theme.textMuted}`}>Paid: {formatCurrency(item.price)} x {item.quantity}</p>
                                            </div>
                                        </div>
                                        {isAdded && (
