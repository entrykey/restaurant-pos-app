...
const PayInSheet = ({ isOpen, onClose, order, onSuccess }) => {
    ...
    useEffect(() => {
        if (isOpen && order) {
            setAmount(String(order.balanceAmount || (order.grandTotal - order.totalPaid)));
    ...
    const balanceAmount = order.balanceAmount || (order.grandTotal - order.totalPaid);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await orderService.addPayment(order.id, {
                ...
                amount: payAmount,
                // Passing existing customer info just for logging consistency
    ...
    const paymentMethods = [
        { id: "cash", label: "Cash", icon: Banknote, color: theme.mode === 'dark' ? "text-emerald-400 bg-emerald-900/30" : "text-green-600 bg-green-50" },
        { id: "card", label: "Card", icon: CreditCard, color: theme.mode === 'dark' ? "text-blue-400 bg-blue-900/30" : "text-blue-600 bg-blue-50" },
        { id: "upi", label: "UPI", icon: Smartphone, color: theme.mode === 'dark' ? "text-orange-400 bg-orange-900/30" : "text-orange-600 bg-orange-50" },
        { id: "other", label: "Other", icon: Receipt, color: theme.mode === 'dark' ? "text-slate-400 bg-slate-800" : "text-gray-600 bg-gray-100" },
    ];
            ...
            <div className={`relative w-full max-w-md h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 ${theme.surfaceBg}`}>
                ...
                <div className={`px-6 py-5 border-b flex items-center justify-between ${theme.borderLight}`}>
                    <div>
                        ...
                        <p className={`text-sm font-medium ${theme.textMuted}`}>{order.orderNumber}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    <div className={`${theme.pageBg} rounded-2xl p-5 border ${theme.borderLight} space-y-3`}>
                        <div className="flex justify-between items-center text-sm">
                            <span className={`${theme.textMuted} font-medium`}>Customer</span>
                        ...
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <span className={`font-bold ${theme.textHeading}`}>{formatCurrency(order.grandTotal)}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                            <span className={`${theme.textMuted} font-medium`}>Already Paid</span>
                            <span className="font-bold text-green-600">{formatCurrency(order.totalPaid)}</span>
                        </div>
                        <div className={`pt-3 border-t ${theme.borderLight} flex justify-between items-center`}>
                            ...
                            <span className="text-xl font-black text-orange-600">{formatCurrency(balanceAmount)}</span>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {paymentMethods.map((method) => (
                                ...
                                >
                                    ...
                                    <span className={`font-bold text-sm ${selectedMethod?.id === method.id ? 'text-indigo-400' : theme.textHeading}`}>
                                        {method.label}
                                    </span>
                                </button>
                            ))}
                        ...
                        <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className={`w-full p-4 pl-10 rounded-xl border outline-none focus:ring-2 focus:ring-indigo-500 font-black text-xl ${theme.surfaceBg} ${theme.textHeading} ${theme.borderLight}`}
                                    placeholder="0.00"
                                    autoFocus
                                />
                            </div>
                ...
                <div className={`p-6 border-t stretch ${theme.borderLight} ${theme.pageBg}`}>
                    <button
                        ...
                        className="w-full py-4 rounded-xl text-white font-black text-lg transition-all flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                    >
                        {isSubmitting ? (
